...
import 'package:frontend/services/instagram_service.dart';
import 'package:frontend/services/nearby_users_service.dart';
...
class ReportLostItemScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const ReportLostItemForm();
  }
}

class ReportLostItemForm extends StatefulWidget {
  const ReportLostItemForm({super.key});

  @override
  State<ReportLostItemForm> createState() => _ReportLostItemFormState();
}

class _ReportLostItemFormState extends State<ReportLostItemForm> {
  ...
  List<String> _selectedCategories = ['Electronics'];
  ...
  Future<void> _analyzeImageWithAI() async {
    ...
    try {
      ...
      final result = await geminiService.analyzeImage(_image!);
      ...
      setState(() {
        ...
        if (_categories.contains(result.suggestedCategory) &&
            !_selectedCategories.contains(result.suggestedCategory)) {
          _selectedCategories = [result.suggestedCategory];
        }
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Expanded(
                  child: Text('AI analyzed: ${result.tags.take(3).join(", ")}'),
                ),
              ],
            ),
  ...
  /// Search for places when user types in location field
  Future<void> _onLocationSearch(String query) async {
    ...
    try {
      ...
      print('📍 showPlacePredictions=$_showPlacePredictions');
    } catch (e) {
  ...
  /// Select a place from autocomplete predictions
  Future<void> _selectPlace(PlacePrediction prediction) async {
    ...
    // Get place details to get lat/long
    try {
  ...
  Future<void> _submitForm() async {
    ...
    if (_locationData == null && _locationController.text.trim().isEmpty) {
      _showErrorSnackBar('Please add the location where it was lost');
      return;
    }
    ...
    try {
      ...
      if (_selectedCategories.contains('People')) {
        ...
        if (_personWhereaboutsController.text.isNotEmpty) {
          personDetails.add('Last seen: ${_personWhereaboutsController.text}');
        }
      ...
      // Create the item
      print('🏷️ Tags being saved: $_generatedTags');
      final itemDescription = _descriptionController.text.isNotEmpty
          ? _descriptionController.text
          : fullDescription;
      final item = Item(
        id: '', // Will be set by Firestore
        description: '${_titleController.text}|||$itemDescription',
        latitude: latitude,
        longitude: longitude,
        ...
        tags: _generatedTags.isNotEmpty ? _generatedTags : null,
      );
      print('📦 Item tags: ${item.tags}');

      // Create item with image URL and save
      final itemWithImage = item.copyWith(imageUrl: imageUrl);
      await context.read<ItemsRepository>().addItemDirect(itemWithImage);

      // Alert nearby users who were in the area
      final nearbyService = NearbyUsersService();
      final alertsSent = await nearbyService.alertNearbyUsers(
        latitude: latitude,
        longitude: longitude,
        incidentTime: DateTime.now(),
        itemDescription: '${_titleController.text}\n$fullDescription',
        itemId: '', // Item ID not available here, handled by service
      );
      ...
      if (imageUrl != null) {
        try {
          ...
          final success = await instagramService.postLostItem(
            imageUrl: imageUrl,
          ...
          instagramResult = success ? 'Posted to Instagram!' : null;
        } catch (e) {
      ...
      if (alertsSent > 0) {
        _showSuccessSnackBar(
          'Report submitted! 🔔 $alertsSent nearby users alerted${instagramResult != null ? ' 📸 $instagramResult' : ''}',
        );
      } else if (instagramResult != null) {
        _showSuccessSnackBar('Report submitted! 📸 $instagramResult');
      } else {
  ...
  void _showAuthRequiredDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'You need to sign in to report lost items. This helps others contact you when they find your item.',
        ),
        actions: [
          TextButton(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Fill in the details to help others find it',
              ...
              Text(
                'Add tags to help others find this item',
                    ...
                    child: TextField(
                      ...
                      onSubmitted: (value) {
                        if (value.trim().isNotEmpty &&
                            !_generatedTags.contains(value.trim())) {
                          setState(() {
                            _generatedTags.add(value.trim());
                  ...
                  ElevatedButton(
                    onPressed: () {
                      if (_tagController.text.trim().isNotEmpty &&
                          !_generatedTags.contains(
                            _tagController.text.trim(),
                          )) {
                        setState(() {
                          _generatedTags.add(_tagController.text.trim());
                          _tagController.clear();
                        });
                      }
                    },
              ...
              if (_generatedTags.isNotEmpty)
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.auto_awesome, color: Colors.grey.shade400),
                      const SizedBox(width: 8),
                      Text(
                        'Upload an image for AI-generated tags',
              ...
              const SizedBox(height: 8),
              // Show partial speech text while listening
